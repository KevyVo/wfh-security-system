...
class lock:
    def __init__(self, RED, GREEN, REDIN, GREENIN, LOCK, BUZZ_OPEN, BUZZ_CLOSE, REED_SWITCH, thing, ip):
        ...
        #Phue object
        self.b = Bridge(ip)#Your light ip
    ...
    def access(self, userID, finger, accuracyScore):
      ...
      while(GPIO.input(self.Reed) == 1):
        if (openonce == 0):
          ...
          self.thing.publish("Smart_Lock", self.thing.createPayload(userID, finger, accuracyScore, self.getReedStatus(), dt_string, unix, date))
          del now
          openonce = 1
        pass
    ...
    def inExit(self):
      ...
      #Enter a loop of Door logic

        # GPIO.input(self.Reed) == 0 means DOOR IS CLOSED
        # GPIO.input(self.Reed) == 1 means DOOR IS OPENED
        # openonce is a flag for if the door has even been open, set to FALSE at the start
        # looponce is a flag to control the execution of the log so it only happens once, set to False at the start
      print("The door is enter a while loop and Door status is:", GPIO.input(self.Reed))
      while True:
        #1 This mean the door is close and the door has already been open
        if (GPIO.input(self.Reed) == 0 and openonce == True):
          #Only use for debugging
          #print("This loop will break")
          break
        
        #2 This means the door is closed, used to enter a waiting state if the door has never been open once yet
        if (GPIO.input(self.Reed) == 0):
          pass
        
        #3 This means the door was instially open or has just change states to be open
        if (GPIO.input(self.Reed) == 1):
          #Set lights on
          self.b.set_light(1, 'on', True)
          self.b.set_light(2, 'on', True)
          self.b.set_light(1, 'xy', self.c.rgb_to_xy(253, 244, 220))
          self.b.set_light(2, 'xy', self.c.rgb_to_xy(253, 244, 220))
          # This is here so we don`t get like 50 AWS DB LOGS
          if (looponce == False):
              #Only use for debugging
              #print("The door has open once.")
              # datetime object containing current date and time
              now = datetime.now()
              dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
              date = str(now.date())
              ...
              del now
              self.lcd.backlight(False)
              looponce = True #The loop has ran once, change flag status to comfirm
              openonce = True #The Door has now been openonce and will change flag status to comfirm
