...
def noAccess():
    #This method will only be used when the fingerprint is invaild
...
def access():
    #The method will only be used when the fingerprint is vaild
...
while True:
  #Master Buttons
  pir.when_motion = LED_ON2.on
  pir.when_no_motion = LED_ON2.off
  YBUTT2.when_pressed = RELAY.off 
  YBUTT2.when_released = RELAY.on
  GBUTT2.when_pressed = RELAY.off
  GBUTT2.when_released = RELAY.on
  #While loop #1 (Door is open or no motion)
  while ((GPIO.input(Reed)==1) or ((pir.value) == False)):
      GPIO.output(LOCK, False)
      lcd_string('Door Status:',LCD_LINE_1)
      lcd_string('UNLOCK',LCD_LINE_2)
      pass
  while ((flag == False) and ((pir.value) == True) and (GPIO.input(Reed)==0)):
      GPIO.output(LOCK, True)
  #Fingerprint setup and authentication 
      try:
  ...
  ## Tries to search the finger and calculate hash
      try:
          ...
          while((f.readImage() == False) and ((pir.value) == True)):
            flag = False
            etime = f.readImage()
            pass
      except Exception as e:
