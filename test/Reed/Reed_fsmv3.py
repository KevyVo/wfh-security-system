...
#Flag for status of the Lock
lFlag = False

def setup():
  # Main pin setup block
...
while True:
  ...
  while ((lFlag == True) and ((pir.value)==True)):
         try:
          ...
          ## Gets some sensor information
           ...
           ## Store if the Finger has been scan or not
             flag = f.readImage()
  ...
  while((flag == False) and (pir.value==1) and (lFlag==True)):
        GPIO.output(BUZZC, True)
        GPIO.output(RED, True)
        GPIO.output(REDIN, True)
        time.sleep(0.6)
