    ...
    while True:
        ...
        if ((Lock.getReed() == 1)):#Door is open
            ...
            while True:
                ...
                if (f.readImage() == True): #READ if finger is read
                    ...
                    if (positionNumber != -1 and accuracyScore >=50): #Finger is in database if true, if return -1 mean the fingerprint is not in system
        ...
        if(readerState == True):
            ...
            if (f.readImage() == True): #READ if finger is read
                ...
                #Check buffer template against stored template
                if (positionNumber != 0 and accuracyScore >=50): #Finger is in database if true
                    response = DB.getItem(positionNumber)
                    userID = response["userID"]
                    finger = response["finger"]
