...
def access():
      ...
      if (positionNumber == 0):        
        ...
        print('ID#' + str(newPos) + " access: " + dt_string + ", Accuracy: " + str(accuracyScore))
        time.sleep(2)
      elif (positionNumber == 1):
        newPos ="Kevin`s Thumb"
        lcd_string('ID#' + newPos,LCD_LINE_1)
        lcd_string('Accuracy: ' + str(accuracyScore),LCD_LINE_2)
        print('ID#' + newPos + " access: " + dt_string + ", Accuracy: " + str(accuracyScore))
        time.sleep(2)
      ...
      while(GPIO.input(Reed) == 1):
        if (openonce == 0):
          ...
          print('ID#' + str(newPos) + " opened: " + dt_string2)
          openonce = 1
        pass
      relock(newPos)
...
def relock(newPos):
      ...
      dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
      print('ID#' + newPos + " closed: " + dt_string) 
...
def rundownload(f, now):
      # Records the fail thumb print and stores it locally
      print('Downloading image (this take a while)...')
      dt_string = now.strftime("%d_%m_%Y-%H_%M_%S-")
      imageDestination = '/home/pi/Pictures/failPrints/' + dt_string + 'fingerprint.bmp'
      f.downloadImage(imageDestination)
      print('The image was saved to "' + imageDestination + "at the time: " + dt_string + '".')   
#Pin Setup 
setup()
#Lock On
...
while True:
      ...
      if ((pir.value == True) and (GPIO.input(Reed) == 0)):
            ...
            try:  #Intialize finger print reader
              ...
              f2 = PyFingerprint('/dev/ttyUSB0', 57600, 0xFFFFFFFF, 0x00000000)

              if ( (f.verifyPassword() == False) and (f2.verifyPassword() == False)):
            ...
            t1 = threading.Thread(target=rundownload, args=(f, now))   
            if (templates == 0):
      ...
      if(readerState == True):
            readerState = False
            if ((f.readImage() == True) and (f2.readImage() == True)): #READ if finger is read
...
##                       dt_string = now.strftime("%d_%m_%Y-%H_%M_%S-")
##                       imageDestination = '/home/pi/Pictures/failPrints/' + dt_string + 'fingerprint.bmp'
##                       f2.downloadImage(imageDestination)
##                       print('The image was saved to "' + imageDestination + "at the time: " + dt_string + '".')
                      # erase stored results in memory
                       del result
                       del positionNumber
